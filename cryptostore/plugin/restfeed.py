...
class Restfeed(Plugin):
    ...
    def _worker(self, exchange):
        ...
        # for pair in self.config.restfeed[exchange]:
        #     LOG.info("Restfeed - Starting for %s - %s for start %s", exchange, pair, self.config.restfeed[exchange][pair]['start'])

        CFG = os.path.join(os.getcwd(), 'sandbox/rest_config.yaml')

        # public = Rest().binance_delivery
        sandbox = Rest(config=CFG)[exchange]
        for symbol in self.config.restfeed[exchange]['orders']:
            # try:
                while True:

                    try:
                        orders = sandbox.orders(symbol=symbol)
                        LOG.info(orders)
                    ...
                    #     trades.extend(t)
                        ...
                        LOG.warning("Restfeed - encountered error rest api calling %s - %s, trying again...", exchange, symbol, exc_info=True)
                        time.sleep(300)
                        continue
    ...
    def run(self):
        LOG.info("Restfeed running on PID %d", os.getpid())
        if 'restfeed' in self.config:
            for exchange in self.config.restfeed:
